...
POINT arrowCoords(POINT from, POINT to) { // возвращает координаты где нужно строить стрелку
...
POINT loopPOINT(POINT from, POINT to) { // возвращает координаты где нужно строить петлю
    float alpha = atan2(to.y - from.y, to.x - from.x);
    float distance = sqrt(pow((to.x - from.x), 2) + pow((to.y - from.y), 2));
    POINT loop;
...
POINT middleDots(POINT from, POINT to) { // возвращает смещение для построения стрелки (промежуточную точку)
    float alpha = atan2(to.y - from.y, to.x - from.x);
    POINT result;
