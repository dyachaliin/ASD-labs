...
LRESULT CALLBACK defaultGraphProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT:
                ...
                for (int j = 0; j < n; j++) {
                    ...
                    if (A[i][j] == 1) { // если есть связь
                        ...
                        if (i == j) { // петля
                            ...
                            if ((abs(i - j) == 1 || (verts[i].line != verts[j].line && i != 0 && j != 0)) && (i < j || A[j][i] == 0)) { // если соседние вершины или не на одной стороне треугольника
        ...
        for (int i = 1; i < n; i++) { // отрисовка вершин
            char name[3];
            int newN = 0;
