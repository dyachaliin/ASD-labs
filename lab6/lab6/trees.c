...
LRESULT CALLBACK condensationGraphProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_PAINT:
        ...
        // calculate vertices coordinates
        {
            ...
            for (; vertCounter < componentsN; vertCounter++) { // äâèãàåìñÿ ïî îñíîâàíèþ òðåóãîëüíèêà
                newX -= step;
                verts[vertCounter].coords.x = newX;
                verts[vertCounter].coords.y = newY;
        ...
        for (int i = 0; i < componentsN; i++) {
            char ar[17];
            snprintf(ar, 17, "Êîìïîíåíòà %d:\t", i + 1);
            ...
            for (int j = 0; V[i][j] != 0; j++) {
                ...
                int currVert = V[i][j];
                // äëÿ êàæäîãî ýëåìåíòà êîìïîíåíòû ïðîáåæèìñÿ ïî äðóãèì êîìïîíåíòàì
        ...
        { // Âûâîäèì ñâÿçè îðèåíò. ãðàôà
            for (int i = 0; i < componentsN; i++) { // oriented
                for (int j = 0; j < componentsN; j++) {
                    ...
                    if (condMatrix[i][j] == 1) { // åñëè åñòü ñâÿçü
                        if (i == j) { // ïåòëÿ
        ...
        TextOutA(hdc, nonStartX - 170, 100, "Ìàòðèöà ñìåæíîñòè:", 19);
        POINT matrixA = { nonStartX, 100 };
        showMatrix(A, matrixA, hdc);

        TextOutA(hdc, nonStartX + 220, 100, "Ìàòðèöà äîñòèæèìîñòè:", 22);
        POINT matrixDos = { nonStartX + 400, 100 };
        showMatrix(dos, matrixDos, hdc);

        TextOutA(hdc, nonStartX - 170, 350, "Ìàòðèöà ñâÿçíîñòè:", 19);
        POINT matrixZv = { nonStartX, 350 };
        showMatrix(zvMatrix, matrixZv, hdc);

        TextOutA(hdc, nonStartX + 220, 350, "Ìàòðèöà ãð. êîíä.:", 19);
